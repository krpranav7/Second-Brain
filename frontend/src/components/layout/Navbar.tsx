...
export function Navbar({ onMenuClick, onShareClick, onAddClick }: NavbarProps) {
    return (
        <header className='sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/65 backdrop-blur-md px-4 py-3 dark:border-neutral-800 dark:bg-neutral-900/70 md:px-6'>
            <div className='flex items-center gap-3'>
                <button
                    onClick={onMenuClick}
                    aria-label='Open menu'
                    className='cursor-pointer rounded-lg p-2 text-slate-600 hover:bg-slate-100 dark:text-neutral-300 dark:hover:bg-neutral-800 md:hidden'
                >
                    <Menu size={20} />
                </button>
